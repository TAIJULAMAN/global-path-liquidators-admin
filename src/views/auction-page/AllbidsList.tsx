import React, { useRef, ChangeEvent, useState } from "react";
import { FaEdit } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import { AiOutlineDelete } from "react-icons/ai";
import axios from "axios";
import Swal from "sweetalert2";
import { GrFormView } from "react-icons/gr";

const AllbidsList = () => {
  const [page, setPage] = useState(1); 
  const [perPage, setPerPage] = useState(10);
  const tableRef = useRef(null);
  type Auctionstore = {
    id: number;
    product_name: string;
    phone_number: string;
    email: string;
    bid_id: string;
    amount: string;
    business_name: string;
    Description: string;
    bidStatus: string;
    product_image: string;
    category_name: string;
    Title: string;
    manifest_status: string;
    Current_Bid: string;
    metaPage: number;
    Starting_Price: string;
    created_at: string;
    End_Time: string;
    price: string;
    totalbidPrice: string;
  };
  const [meta, setMeta] = useState<Record<string, unknown>>({});
  const metaPage: any = meta?.page;
  const navigate = useNavigate();
  const handleViewBidDetails = (id: number) => {
    navigate(`/bid-details/${id}`);
  };
  const formatAsMMDDYYYYy = (dateString: string | number | Date) => {
    const dateObj = new Date(dateString);
    const month = String(dateObj.getMonth() + 1).padStart(2, "0"); // Add leading zero if needed
    const day = String(dateObj.getDate()).padStart(2, "0"); // Add leading zero if needed
    const year = dateObj.getFullYear();
    return `${year}-${month}-${day}`;
  };
  const [allbid, setBid] = useState<Array<Auctionstore>>([]);
  const totalPrice = allbid?.reduce(
    (total, item) =>
      total + (typeof item?.amount === "number" ? item?.amount : 0),
    0
  );
  const [refetch, setRefetch] = useState(false);
  const [visibility, setVisibility] = useState("");
  const [isChecked, setIsChecked] = useState(false);
  const handleCreatePage = () => {
    navigate("/auction-product-create");
  };
  React.useEffect(() => {
    // if (refetch) {
    const getAllAuctionStore = async () => {
      const response = await fetch(
        `https://darktechteam.com/api/bids/all-bids?page=${page}&limit=${perPage}`
      );
      const data = await response.json();
      console.log(data);
      setBid(data?.result);
      setMeta(data?.meta);
      setRefetch(false);
    };
    getAllAuctionStore();
  }, [refetch, page, perPage]);
  const handleRemove = (id: number) => {
    Swal.fire({
      title: "Do you want delete the product?",
      icon: "warning",
      showCancelButton: true,
      cancelButtonColor: "red",
      confirmButtonText: "Yes,delete it!",
      cancelButtonText: "No,cancel!",
    }).then((result) => {
      {
        if (result.isConfirmed) {
          const apiUrl = `https://darktechteam.com/api/auctions/remove-auction/${id}`;
          axios.delete(apiUrl).then((response) => {
            console.log("DELETE request successful");
            console.log("Response:", response.data);
            alert("Product Deleted successfully");
          });
          setRefetch(true);
        } else if (result.dismiss) {
          Swal.fire("Cancelled", "", "error");
        }
      }
    });
  };
  const handleEditAuction = (id: number) => {
    navigate(`/auction/product-edit/${id}`);
  };
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    setIsChecked(event.target.checked);
    if (!event.target.checked) {
    }
  };
  const [searchQuery, setSearchQuery] = useState("");
  const filteredResults = allbid?.filter((item: any) =>
    (item?.id?.toString().toLowerCase() ?? "").includes(
      searchQuery.toLowerCase()
    )
  );

  return (
    <div>
      <div className=" grid grid-cols-1 md:grid-cols-5 gap-2">
        <div className="card bg-[#304FFE] dark:bg-[#304FFE] text-white p-5">
          <h6 className="text-white">TOTAL BIDDER</h6>
          <h2 className="text-white font-bold">{allbid?.length}</h2>
        </div>
        <div className="card bg-[#039BE5] dark:bg-[#039BE5] text-white p-5">
          <h6 className="text-white">TOTAL AMOUNT</h6>
          <h4 className=" text-white font-bold">${totalPrice}</h4>
        </div>
        <div className="card bg-[#26A69A] dark:bg-[#26A69A] text-white p-5">
          <h6 className="text-white">TOTAL WINNER</h6>
          <h2 className="text-white font-bold">0</h2>
        </div>
        <div className="card bg-[#ff8d00] dark:bg-[#ff8d00] text-white p-5">
          <h6 className="text-white">LIVE AUCTIONS</h6>
          <h2 className=" text-white font-bold">4</h2>
        </div>
        <div className="card bg-[#9575CD] dark:bg-[#9575CD] text-white p-5">
          <h6 className="text-white">CLOSED BIDS</h6>
          <h2 className="text-white font-bold">0</h2>
        </div>
      </div>

      <div className="text-3xl text-natural-900 font-bold my-5 uppercase">
        ALL BID LIST
      </div>
      <div>
        <div className="relative border dark:border-gray-500 rounded-lg">
          <label className="sr-only">Search</label>

          <input
            type="text"
            name="table-with-pagination-search"
            id="table-with-pagination-search"
            className="form-input p-4 ps-11 rounded-lg dark:bg-gray-800 dark:border-gray-500 w-full focus:w-full"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search by Bid ID"
          />
          <div className="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-4">
            <svg
              className="h-3.5 w-3.5 text-gray-400"
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
              fill="currentColor"
              viewBox="0 0 16 16"
            >
              <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" />
            </svg>
          </div>
        </div>
      </div>

      <div className="py-5">
        <div className="overflow-x-auto">
          <div className="min-w-full inline-block align-middle">
            <div className="border rounded-lg divide-y divide-gray-200 dark:border-gray-700 dark:divide-gray-700">
              <div className="min-w-full overflow-hidden">
                <table
                  ref={tableRef}
                  className="min-w-full divide-y divide-gray-200 dark:divide-gray-700"
                >
                  <thead className="bg-[#42A5F5] text-white dark:bg-gray-700">
                    <tr>
                      <th
                        scope="col"
                        className="px-4  text-white py-3 text-center text-xs font-medium  uppercase"
                      >
                        SL NO
                      </th>
                      <th
                        scope="col"
                        className="px-4  text-white py-3 text-center text-xs font-medium  uppercase"
                      >
                        BID NO.
                      </th>
                      <th
                        scope="col"
                        className="px-4  text-white py-3 text-center text-xs font-medium  uppercase"
                      >
                        DATE
                      </th>
                      <th
                        scope="col"
                        className="px-2 py-3 text-center text-xs font-medium text-white uppercase"
                      >
                        PRODUCT IMAGE
                      </th>
                      <th
                        scope="col"
                        className="px-2 py-3 text-center text-xs font-medium text-white uppercase"
                      >
                        PRODUCT NAME
                      </th>
                      <th
                        scope="col"
                        className="px-2 py-3 text-center text-xs font-medium text-white uppercase"
                      >
                        EMAIL
                      </th>

                      <th
                        scope="col"
                        className="px-4 py-3 text-left text-xs font-medium text-white uppercase"
                      >
                        PHONE
                      </th>

                      <th
                        scope="col"
                        className="px-2 py-3 text-left text-xs font-medium text-white uppercase"
                      >
                        STARTING PRICE
                      </th>

                      <th
                        scope="col"
                        className="px-2  py-3 text-left text-xs font-medium text-white uppercase"
                      >
                        BID PRICE
                      </th>

                      <th
                        scope="col"
                        className="px-2  py-3 text-left text-xs font-medium text-white uppercase"
                      >
                        BID STATUS
                      </th>

                      <th
                        scope="col"
                        className="px-4 py-3  text-xs font-medium text-white uppercase"
                      >
                        Action
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {filteredResults?.length > 0 ? (
                      (filteredResults || [])?.map((bid, index: number) => {
                        return (
                          <tr key={index}>
                            <td className=" px-1 text-center py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {index + 1}
                            </td>
                            <td className=" px-1 text-center py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.id}
                            </td>
                            <td className=" px-2 text-center py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {formatAsMMDDYYYYy(bid?.created_at)}
                            </td>
                            <td className=" px-2 justify-center  py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              <img
                                className="text-center ml-4 w-16 h-12 rounded"
                                src={`https://darktechteam.com/api/${bid?.product_image}`}
                                alt=""
                              />
                            </td>
                            <td className=" px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.product_name}
                            </td>
                            <td className=" px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.email}
                            </td>
                            <td className=" px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.phone_number}
                            </td>

                            <td className="px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.Starting_Price}
                            </td>
                            <td className=" px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              {bid?.amount}
                            </td>

                            <td className="px-2 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                              <span className="bg-[#C5CAE9] p-2 px-2 rounded">
                                {bid?.bidStatus}
                              </span>
                            </td>

                            <td className="px-2 mt-2 text-end  py-3  text-[30px] text-indigo-400">
                              <GrFormView
                                className="text-center ml-6"
                                onClick={(e) => {
                                  e.preventDefault();
                                  if (bid?.id) handleViewBidDetails(bid?.id);
                                }}
                              />
                            </td>
                          </tr>
                        );
                      })
                    ) : (
                      <td>
                        <span className="w-full flex justify-center items-center p-5">
                          No data found
                        </span>
                      </td>
                    )}
                  </tbody>
                </table>
              </div>

              {/* Table Footer */}
              <div className="py-1 px-4 flex justify-between">
                {/* limit dropdown */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <label
                      htmlFor="table-pagination-limit"
                      className="text-sm text-natural-900"
                    >
                      Show
                    </label>
                    <select
                      id="table-pagination-limit"
                      className="mx-2 form-select text-sm text-natural-900"
                      value={perPage}
                      onChange={(e) => setPerPage(parseInt(e.target.value))}
                    >
                      <option value="10">10</option>
                      <option value="25">25</option>
                      <option value="50">50</option>
                      <option value="100">100</option>
                      <option value="250">250</option>
                      <option value="500">500</option>
                    </select>
                    <label
                      htmlFor="table-pagination-limit"
                      className="text-sm text-gray-700 dark:text-gray-200"
                    >
                      Entries
                    </label>
                  </div>
                </div>
                {/* Pagination */}
                <nav className="flex items-center justify-end space-x-2">
                  <button
                    className="text-gray-400 hover:text-primary p-4 inline-flex items-center gap-2  rounded-md"
                    disabled={metaPage === 1}
                    onClick={(e) => setPage(page - 1)}
                  >
                    <span aria-hidden="true">«</span>
                    <span className="sr-only">Previous</span>
                  </button>
                  {metaPage}
                  <button
                    className="text-gray-400 hover:text-primary p-4 inline-flex items-center gap-2  rounded-md"
                    onClick={() => setPage(page + 1)}
                  >
                    <span className="sr-only">Next</span>
                    <span aria-hidden="true">»</span>
                  </button>
                </nav>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AllbidsList;
